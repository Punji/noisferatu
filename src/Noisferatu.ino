...
const uint8_t algoCountPerBank[BANK_COUNT] = {
  ...
  TALKIE_ALGO_COUNT   // Bank 6: 9 Talkie
};
...
// --------------------------------------------------
// TalkiePCM audio callback
// --------------------------------------------------
void audioCallback(int16_t* data, int len) {
  for (int i = 0; i < len; i++) {
    uint16_t nextWrite = (writeIndex + 1) % AUDIO_BUFFER_SIZE;
    if (nextWrite != readIndex) {  // avoid overwrite
...
void setup()
{
  ...
  // Initialize TalkiePCM
  voice.setDataCallback(audioCallback);
  voice.setChannels(1);
...
void updateParams()
{
  ...
  if (currentBank == BANK_NOISY_TEXTURES) {
    switch (currentAlgo) {
...
void loop()
{
  ...
  // Generate a Talkie samples every 25ms
  if (currentBank == BANK_TALKIE) {
    if (samplesAvailable() < AUDIO_MIN_SAMPLES) {
      ...
      do { 
        switch (currentAlgo) {
          case TALKIE_ALGO_1:
            f = generateTalkieFrame1();
            break;
          case TALKIE_ALGO_2:
            f = generateTalkieFrame2();
            break;
          case TALKIE_ALGO_3:
            f = generateTalkieFrame3();
            break;
          case TALKIE_ALGO_4:
            f = generateTalkieFrame4();
            break;
          case TALKIE_ALGO_5:
            f = generateTalkieFrame5();
            break;
          case TALKIE_ALGO_6:
            f = generateTalkieFrame6();
            break;
          case TALKIE_ALGO_7:
            f = generateTalkieFrame7();
            break;
          case TALKIE_ALGO_8:
            f = generateTalkieFrame8();
            break;
          case TALKIE_ALGO_9:
            f = generateTalkieFrame9();
            break;
        }
      } while (f.energy == 0xF);
...
void TC5_Handler()
{
  if (TC5->COUNT16.INTFLAG.bit.MC0) {
    ...
    analogWrite(DAC_PIN, DAC_CENTER + volumed + 1);
